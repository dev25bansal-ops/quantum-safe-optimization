    ...
    async def record_failure(self):
        ...
        async with self._lock:
            if self._state == CircuitState.HALF_OPEN:
                ...
                print(f"🔴 Circuit breaker OPEN - failure in half-open state")
            elif self._state == CircuitState.CLOSED:
...
class CosmosDBManager:
    ...
    def configure(
        self,
        retry_policy: Optional[CosmosRetryPolicy] = None,
        pool_config: Optional[CosmosPoolConfig] = None,
    ):
    ...
    async def initialize(self):
        ...
        print(f"✅ Cosmos DB initialized with pool (max_conn={self.pool_config.max_connections})")
        ...
        for attempt in range(self.retry_policy.max_retries + 1):
            try:
                ...
                if attempt < self.retry_policy.max_retries:
                    ...
                    print(f"⚠️ Cosmos DB retry {attempt + 1}/{self.retry_policy.max_retries}: "
                          f"status={e.status_code}, waiting {wait_time:.2f}s")
    ...
    async def close(self):
        ...
        print("✅ Cosmos DB connections closed")
...
class JobRepository:
    ...
    async def get_job_any_user(self, job_id: str) -> Optional[Dict[str, Any]]:
...
class KeyRepository:
    ...
    async def get_keys(self, user_id: str) -> Optional[Dict[str, Any]]:
