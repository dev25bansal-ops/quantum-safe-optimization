...
class InMemoryJobStore(BaseStore[Dict[str, Any]]):
    ...
    async def get(self, item_id: str, partition_key: str) -> Optional[Dict[str, Any]]:
        job = self._data.get(item_id)
        if job and job.get("user_id") == partition_key:
            return job
        return None
    
    async def get_any_partition(self, item_id: str) -> Optional[Dict[str, Any]]:
...
class InMemoryUserStore(BaseStore[Dict[str, Any]]):
    ...
    async def update(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if user_id in self._data:
            ...
            old_username = old_user.get("username") if old_user is not data else None
...
class InMemoryTokenStore(BaseStore[Dict[str, Any]]):
    ...
    async def get(self, item_id: str, partition_key: str = None) -> Optional[Dict[str, Any]]:
        return self._data.get(item_id)
...
_job_store: Optional[BaseStore] = None
_user_store: Optional[BaseStore] = None
_key_store: Optional[BaseStore] = None
_token_store: Optional[BaseStore] = None
...
async def get_user_store() -> BaseStore[Dict[str, Any]]:
    ...
    if await _try_init_cosmos():
        try:
            from api.db.cosmos import cosmos_manager, UserRepository
...
async def get_key_store() -> BaseStore[Dict[str, Any]]:
    ...
    if await _try_init_cosmos():
        try:
            from api.db.cosmos import cosmos_manager, KeyRepository
