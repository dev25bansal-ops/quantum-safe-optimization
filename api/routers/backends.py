...
class DeviceInfo(BaseModel):
    ...
    provider: Optional[str] = Field(None, description="Hardware provider")
    num_qubits: Optional[int] = Field(None, description="Number of qubits")
    status: Optional[str] = Field(None, description="Device operational status")
    device_type: Optional[str] = Field(None, description="Device type (QPU, simulator)")
    region: Optional[str] = Field(None, description="Geographic region")
    cost_per_shot: Optional[float] = Field(None, description="Approximate cost per shot")
...
async def get_all_metrics(
    ...
    return MetricsResponse(
        metrics={
            k: BackendMetrics(**v) for k, v in all_metrics.items()
        }
    )
...
class BackendConfigRequest(BaseModel):
    ...
    device_name: Optional[str] = Field(None, description="Default device to use")
