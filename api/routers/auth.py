...
# Import PQC crypto
from quantum_safe_crypto import SigningKeyPair, KemKeyPair, py_sign, py_verify
...
try:
    from api.db.repository import get_user_store, get_key_store, get_token_store
    _db_available = True
except ImportError:
    _db_available = False
    get_user_store = None
...
_server_signing_keypair: Optional[SigningKeyPair] = None
...
class UserRegistration(BaseModel):
    ...
    kem_public_key: Optional[str] = Field(None, description="ML-KEM-768 public key for result encryption")
...
async def get_user_keys(user_id: str) -> Optional[dict]:
...
def create_pqc_token(user_id: str, username: str, roles: list[str], signing_keypair: SigningKeyPair) -> tuple[str, str]:
    """
...
def verify_pqc_token(token: str, signing_keypair: Optional[SigningKeyPair] = None) -> Optional[dict]:
    """
...
async def get_key_info(
    ...
    return {
        "key_id": key_id,
        "created_at": key_info["created_at"].isoformat() if isinstance(key_info.get("created_at"), datetime) else key_info.get("created_at", "unknown"),
        "expires_at": key_info.get("expires_at", "never").isoformat() 
            if isinstance(key_info.get("expires_at"), datetime) else "never",
...
async def revoke_all_user_tokens(
    ...
    """
    user_id = current_user.get("sub")
    count = await token_revocation_service.revoke_all_user_tokens(
        user_id,
