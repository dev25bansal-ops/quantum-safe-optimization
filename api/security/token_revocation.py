...
class TokenRevocationService:
    ...
    async def initialize(self, config: Optional[TokenRevocationConfig] = None):
        ...
        try:
            ...
            print("✅ Token revocation service connected to Redis")
        except Exception as e:
            if self.config.use_memory_fallback:
                print(f"⚠️ Redis unavailable, using memory fallback for token revocation: {e}")
    ...
    async def revoke_token(
        ...
        if self._use_memory:
            ...
            self._memory_expiry[token_jti] = datetime.utcnow() + timedelta(seconds=ttl)
            return True
