...
class TelemetrySettings(BaseSettings):
    ...
    app_insights_connection: Optional[str] = Field(
        default=None,
...
class QuantumBackendSettings(BaseSettings):
    ...
    aws_braket_bucket: Optional[str] = Field(
        default=None,
    ...
    # D-Wave
    dwave_token: Optional[str] = Field(
        default=None,
    ...
    def validate_production_settings(self):
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if not self.security.key_vault_uri:
                import warnings
                warnings.warn(
                    "KEY_VAULT_URI not set in production - using local secrets"
                )
            ...
            if (
                ...
                raise ValueError(
                    "Must change JWT_SECRET from default in production"
                )
