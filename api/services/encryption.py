...
class EncryptedResult:
    ...
    key_id: Optional[str] = None  # Reference to the key used
...
class MLKEMEncryptionService:
    ...
    def __init__(self, default_security_level: int = 3):
        ...
        logger.info(
            "ml_kem_service_initialized",
            extra={"default_level": default_security_level}
        )
    
    def generate_keypair(self, security_level: Optional[int] = None) -> Dict[str, str]:
        """
...
def get_supported_algorithms() -> Dict[str, Any]:
    ...
    return {
        "kem_algorithms": [
            {"level": level, "name": kem, "dsa": dsa}
            for level, kem, dsa in levels
        ],
