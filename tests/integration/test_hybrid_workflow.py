...
class TestHybridOptimizationLoop:
    ...
    def simple_problem(self):
        ...
        variables = [
            Variable(name="theta", var_type=VariableType.CONTINUOUS, lower_bound=0, upper_bound=2*np.pi),
            Variable(name="phi", var_type=VariableType.CONTINUOUS, lower_bound=0, upper_bound=2*np.pi),
        ]
...
class TestSecureJobWorkflow:
    ...
    def test_encrypt_and_sign_job(self):
        ...
        # Generate keys
        tenant_key = crypto.generate_kem_keypair("tenant-1")
        platform_sign_key = crypto.generate_signing_keypair("platform-sign")
