...
from ...domain.models.result import OptimizationResult, ConvergenceInfo
...
class SimulatedAnnealing(BaseClassicalOptimizer):
    ...
    def optimize(
        ...
        *,
        ...
        while temperature > self.config.final_temperature and total_iterations < self.config.max_iterations:
            ...
            # Reheating
            if (self.config.reheating_enabled and 
                iterations_without_improvement > self.config.reheating_threshold):
        ...
        param_dict = {
            problem.variables[i].name: float(best[i])
        ...
        return OptimizationResult(
            ...
            convergence=ConvergenceInfo(
                ...
                reason="completed" if temperature <= self.config.final_temperature else "max_iterations",
            ),
            metadata={
                "algorithm": self.name,
