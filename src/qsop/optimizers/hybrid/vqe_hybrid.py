...
class HybridVQEOptimizer:
    ...
    def optimize(
        ...
        *,
        ...
        current_layers = self.config.min_layers if self.config.adaptive_depth else self.config.ansatz_layers
        ...
        def get_current_tolerance(iter_count: int) -> float:
            if self.config.adaptive_tolerance:
                progress = min(1.0, iter_count / self.config.max_iterations)
                return self.config.initial_tol * (self.config.final_tol / self.config.initial_tol) ** progress
            return self.config.convergence_threshold

        def energy_function(params: NDArray) -> float:
            """Compute energy expectation value."""
            nonlocal iteration, total_shots_used, function_evals, budget_exceeded, last_best_energy, plateau_count
            ...
            metrics.classical_execution_time.labels(algorithm=self.name, backend=self.backend.name if self.backend else "unknown", tenant_id=tenant_id).observe(max(0, classical_time))
            metrics.hybrid_loop_duration.labels(algorithm=self.name, backend=self.backend.name if self.backend else "unknown", tenant_id=tenant_id).observe(energy_end - loop_start)
            
            quantum_results.append(QuantumExecutionResult(
                counts=result.counts,
                shots=current_shots,
                metadata={"params": params.tolist(), "iteration": iteration, "layers": current_layers},
        ...
        def gradient_wrapper(p: NDArray) -> NDArray:
            ...
            needed_shots = 2 * len(p) * (current_shots // 2) if self.config.gradient_method == GradientMethod.PARAMETER_SHIFT else 2 * (current_shots // 2)
            if self.config.shot_budget and (total_shots_used + needed_shots) > self.config.shot_budget:
                budget_exceeded = True
                return np.zeros_like(p)
            
            if self.config.gradient_method == GradientMethod.PARAMETER_SHIFT:
                grad = self._parameter_shift_gradient_adaptive(p, n_qubits, current_layers, problem, context, current_shots)
                total_shots_used += needed_shots
            elif self.config.gradient_method == GradientMethod.SPSA:
                grad = self._spsa_gradient_adaptive(p, n_qubits, current_layers, problem, context, iteration, current_shots)
                total_shots_used += needed_shots
            else:
        ...
        while current_layers <= self.config.max_layers:
            ...
            if (self.config.adaptive_depth and 
                current_layers < self.config.max_layers and 
                not budget_exceeded and
                (plateau_count >= self.config.plateau_patience or iteration < self.config.max_iterations)):
                ...
                new_params[len(res.x):] = np.random.uniform(-0.01, 0.01, size=new_n_params - len(res.x))
        ...
        return OptimizationResult(
            ...
            optimal_parameters={f"p{i}": float(v) for i, v in enumerate(final_result.x)} if final_result else {},
            iterations=iteration,
            function_evaluations=function_evals,
            gradient_evaluations=gradient_evals,
            convergence=ConvergenceInfo(
                converged=(final_result.success if final_result and hasattr(final_result, 'success') else False) and not budget_exceeded,
    ...
    def _spsa_gradient_adaptive(
        ...
        circuit_plus = self._build_ansatz(n_qubits, params_plus, n_layers=n_layers)
        result_plus = self.backend.run(circuit_plus, shots=current_shots // 2, options=context.get("backend_options", {}))
        energy_plus = self._compute_energy(result_plus, problem)
        
        params_minus = params - c * delta
        circuit_minus = self._build_ansatz(n_qubits, params_minus, n_layers=n_layers)
        result_minus = self.backend.run(circuit_minus, shots=current_shots // 2, options=context.get("backend_options", {}))
        energy_minus = self._compute_energy(result_minus, problem)
