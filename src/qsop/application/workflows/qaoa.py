...
class MaxCutProblem:
    ...
    def to_optimization_problem(self) -> OptimizationProblem:
        ...
        variables = [
            Variable(name=f"x_{i}", var_type=VariableType.BINARY) for i in range(self.n_nodes)
        ]
...
class QAOAWorkflow:
    ...
    def _extract_best(self, problem: OptimizationProblem) -> str:
        ...
        for bitstring, _count in self._last_result.counts.items():
            # Qiskit uses little-endian for bitstrings
            solution = [int(b) for b in bitstring[::-1]]
            cost = problem.evaluate(solution)
            if cost < min_cost:
