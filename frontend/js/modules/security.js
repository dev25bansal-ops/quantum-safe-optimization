...
export function initSecurityTests() {
  ...
  document
    .getElementById("test-encrypt")
    ?.addEventListener("click", testEncrypt);

  // Key management buttons
  document
    .getElementById("btn-generate-keys")
    ?.addEventListener("click", generateMLKEMKeys);
  document
    .getElementById("btn-register-key")
    ?.addEventListener("click", registerPublicKey);
}
...
export async function generateMLKEMKeys() {
  ...
  try {
    ...
    document.getElementById("generated-private-key").value =
      data.private_key || data.secret_key || "";
    resultSection.style.display = "block";

    showToast(
      "success",
      "Keys Generated",
      "Your ML-KEM-768 keypair has been created",
    );
    ...
    showToast(
      "error",
      "Generation Failed",
      error.message || "Failed to generate keys",
    );
  } finally {
...
export async function registerPublicKey() {
  ...
  if (!STATE.isAuthenticated) {
    showToast(
      "error",
      "Registration Failed",
      "Please sign in to register keys",
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      "error",
      "Registration Failed",
      error.message || "Failed to register key",
    );
  } finally {
    registerBtn.disabled = false;
    registerBtn.innerHTML =
      '<i class="fas fa-cloud-upload-alt"></i> Register Key';
  }
}
...
window.loadCryptoStatus = loadCryptoStatus;

/**
 * Load and display crypto status from API
 */
export async function loadCryptoStatus() {
  try {
    ...
    if (!response.ok) {
      throw new Error("Crypto status endpoint not available");
    }

    const data = await response.json();
    updateCryptoStatusUI(data);
  } catch (error) {
    console.error("[Crypto] Failed to load crypto status:", error);
    showCryptoStatusFallback();
  }
}

/**
 * Update crypto status UI elements
 */
function updateCryptoStatusUI(data) {
  const statusBadge = document.getElementById("crypto-status-badge");
  const kemStatus = document.getElementById("kem-status");
  const dsaStatus = document.getElementById("dsa-status");
  const kemAlgo = document.getElementById("kem-algo");
  const dsaAlgo = document.getElementById("dsa-algo");
  const kemPkSize = document.getElementById("kem-pk-size");
  const dsaSigSize = document.getElementById("dsa-sig-size");
  const cryptoWarning = document.getElementById("crypto-warning");
  const cryptoWarningText = document.getElementById("crypto-warning-text");

  // Update status badge
  if (statusBadge) {
    if (data.status === "healthy") {
      statusBadge.textContent = "PRODUCTION";
      statusBadge.className = "health-badge healthy";
    } else {
      statusBadge.textContent = "STUB";
      statusBadge.className = "health-badge degraded";
    }
  }

  // Update KEM info
  if (data.algorithms?.kem) {
    const kem = data.algorithms.kem;
    if (kemStatus) {
      kemStatus.textContent = "ACTIVE";
      kemStatus.className = "crypto-algo-badge active";
    }
    if (kemAlgo) kemAlgo.textContent = kem.name || "ML-KEM-768";
    if (kemPkSize) kemPkSize.textContent = kem.public_key_size || "1184";
  }

  // Update DSA info
  if (data.algorithms?.signature) {
    const sig = data.algorithms.signature;
    if (dsaStatus) {
      dsaStatus.textContent = "ACTIVE";
      dsaStatus.className = "crypto-algo-badge active";
    }
    if (dsaAlgo) dsaAlgo.textContent = sig.name || "ML-DSA-65";
    if (dsaSigSize) dsaSigSize.textContent = sig.signature_size || "3293";
  }

  // Show warning if using stub
  if (cryptoWarning && cryptoWarningText) {
    if (data.status !== "healthy") {
      cryptoWarning.style.display = "flex";
      cryptoWarningText.textContent =
        "Using stub implementation - NOT for production. Install liboqs for real PQC algorithms.";
    } else {
...
/**
 * Show fallback crypto status when API is unavailable
 */
function showCryptoStatusFallback() {
  const statusBadge = document.getElementById("crypto-status-badge");
  const kemStatus = document.getElementById("kem-status");
  const dsaStatus = document.getElementById("dsa-status");
  const kemAlgo = document.getElementById("kem-algo");
  ...
  if (statusBadge) {
    ...
    statusBadge.className = "health-badge unhealthy";
  }
  if (kemStatus) {
    kemStatus.textContent = "UNKNOWN";
    kemStatus.className = "crypto-algo-badge stub";
  }
  if (dsaStatus) {
    dsaStatus.textContent = "UNKNOWN";
    dsaStatus.className = "crypto-algo-badge stub";
  }
  if (kemAlgo) kemAlgo.textContent = "--";
  if (dsaAlgo) dsaAlgo.textContent = "--";
  if (kemPkSize) kemPkSize.textContent = "--";
  if (dsaSigSize) dsaSigSize.textContent = "--";
}
